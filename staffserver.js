...
app.post("/payrollfunction", (req, res) => {
  ...
  // Convert string inputs to numbers safely, default to 0 if invalid
  const safeTotalHours = parseFloat(totalHours) || 0;
  const safeHourlyRate = parseFloat(hourlyRate) || 0;
  const safeDeductions = parseFloat(deductions) || 0;

  // Calculate gross and net salary
  const grossSalary = safeTotalHours * safeHourlyRate;
  const netSalary = grossSalary - safeDeductions;

  const sql = `
    INSERT INTO tbl_payroll 
    (staffCode, fullName, salaryYear, payMonth, workingDays, leaveDays, totalHours, hourlyRate, grossSalary, deductions, netSalary, paymentStatus) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
...
app.put("/payrollfunction/:id", (req, res) => {
  ...
  // Prevent updating auto-generated timestamp
  delete updateData.createdAt;

  const sql = `UPDATE tbl_payroll SET ? WHERE id = ?`;

  pool.query(sql, [updateData, payrollId], (err, result) => {
    if (err) {
